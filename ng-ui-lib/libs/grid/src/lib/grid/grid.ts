...
export class NgUiGridComponent implements OnInit, OnDestroy, AfterViewInit {
   ...
   */
  @Output() ngUiGridEvent = new EventEmitter<GridEventType>();
  
  /**
   * Event emitter specifically for cell clicks
   */
  @Output() ngUiGridCellClick = new EventEmitter<CellClickEvent>();
  
  /**
   * Event emitter specifically for row selection changes
   */
  @Output() ngUiGridRowSelect = new EventEmitter<RowSelectEvent>();
  
  /**
   * Event emitter specifically for column sorting
   */
  @Output() ngUiGridColumnSort = new EventEmitter<ColumnSortEvent>();
  
  /**
   * Event emitter specifically for column resizing
   */
  @Output() ngUiGridColumnResize = new EventEmitter<ColumnResizeEvent>();
  ...
  onCellClick(rowIndex: number, columnId: string, value: any, rowData: any, _event: MouseEvent): void {
    ...
    this.ngUiGridCellClick.emit(cellClickEvent);
    this.ngUiGridEvent.emit(cellClickEvent);
  }
  ...
  toggleRowSelection(rowIndex: number): void {
    ...
    this.ngUiGridRowSelect.emit(rowSelectEvent);
    this.ngUiGridEvent.emit(rowSelectEvent);
  }
  ...
  onColumnHeaderClick(column: ColumnDefinition, event: MouseEvent): void {
    ...
    this.ngUiGridColumnSort.emit(columnSortEvent);
    this.ngUiGridEvent.emit(columnSortEvent);
  }
  ...
  private onResizeEnd = (event: MouseEvent): void => {
    ...
    this.ngUiGridColumnResize.emit(columnResizeEvent);
    this.ngUiGridEvent.emit(columnResizeEvent);
  ...
  startEdit(rowIndex: number, columnId: string, currentValue: any): void {
    ...
    setTimeout(() => {
      const editorInput = document.querySelector('.ngui-grid-editor-input, .ngui-grid-editor-select') as HTMLInputElement;
      if (editorInput) {
        editorInput.focus();
  ...
  commitEdit(): void {
    ...
    if (rowData) {
      ...
      this.ngUiGridEvent.emit(cellEditEvent);
    }
  ...
  goToPage(page: number): void {
    ...
    this.ngUiGridEvent.emit(paginationEvent);
   ...
   */
  exportToPdf(filename?: string, options?: Partial<PdfExportOptions>): void {
    const pdfOptions = {
      ...this.exportService.getDefaultPdfOptions(),
      ...options
    };
    ...
    const exportOptions: ExportOptions = {
      ...
      includeHeaders: true,
      dataScope: 'visible',
      formatOptions: pdfOptions
    };
    
    this.onExportRequested(exportOptions);
  }

  /**
   * Export visible data to Google Sheets
   */
  exportToGoogleSheets(filename?: string, options?: Partial<GoogleSheetsOptions>): void {
    const gsOptions = {
      ...this.exportService.getDefaultGoogleSheetsOptions(),
      ...options,
      title: filename || 'grid-export'
    };
   ...
   */
  exportWithTemplate(templateId: string, filename?: string): void {
    const template = this.exportService.getTemplate(templateId);
    if (!template) {
    ...
    const options: ExportOptions = {
      format: template.supportedFormats[0], // Use first supported format
      filename: filename || 'grid-export',
      includeHeaders: true,
      dataScope: 'visible',
      template: template,
      formatOptions: this.getDefaultOptionsForFormat(template.supportedFormats[0])
    };
   ...
   */
  private getDefaultOptionsForFormat(format: string): any {
    switch (format) {
      case 'csv':
        return this.exportService.getDefaultCsvOptions();
      case 'excel':
        return this.exportService.getDefaultExcelOptions();
      case 'pdf':
        return this.exportService.getDefaultPdfOptions();
      case 'google-sheets':
        return this.exportService.getDefaultGoogleSheetsOptions();
      default:
        return {};
    }
  }
