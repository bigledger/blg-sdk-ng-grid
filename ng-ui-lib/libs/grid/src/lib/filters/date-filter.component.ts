 ...
 * 
 * Provides date-based filtering functionality with:
 * - Multiple comparison operators (equals, before, after, between)
 * - Date range filtering
 * - Input validation
 * - Keyboard accessibility
 */
@Component({
  selector: 'ng-ui-date-filter',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: `
    <div class="blg-date-filter">
      <select 
      ...
      <div class="blg-filter-inputs">
        ...
        @if (filterValue().operator === 'between') {
          ...
          <input 
            ...
            (change)="onValue2Change($event)"
            (keydown)="onKeyDown($event)"
            [attr.aria-label]="ariaLabel + ' end date'">
        }
      </div>
...
export class DateFilterComponent {
  @Input() ariaLabel = 'Date filter';
  
  @Output() filterChange = new EventEmitter<DateFilterValue | null>();

  readonly filterValue = signal<DateFilterValue>({
    operator: 'eq',
    value: null,
    value2: null
  });
  ...
  onOperatorChange(event: Event): void {
    ...
    const operator = target.value as DateFilterValue['operator'];
  ...
  onValue2Change(event: Event): void {
    const target = event.target as HTMLInputElement;
    const value2 = target.value || null;
  ...
  private emitFilterChange(): void {
    const current = this.filterValue();
    const hasValidValue = !!current.value || 
                         (current.operator === 'between' && !!current.value2);
